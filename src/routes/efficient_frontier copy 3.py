...
def efficient_frontier():
    ...
    for combo in itertools.combinations(range(len(mean_returns)), 2):
        for end in range(2, min(len(combo) + 1, 3)):  # Ensure min 7 and max 20 funds in a portfolio
            ...
            for i in range(nombreportefeuille):
               ...
               #   if np.all(weights >= 0.02) and np.all(weights <= 0.30) and category_constraints(weights) and additional_constraints(weights):  # Check all constraints
                if np.all(weights >= min_return) and np.all(weights <= max_return) and additional_constraints(weights):  # Check all constraints
