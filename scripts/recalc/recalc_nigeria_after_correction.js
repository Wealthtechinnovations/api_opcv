#!/usr/bin/env node
/**
 * recalc_nigeria_after_correction.js — Recalcul CIBLE apres la correction des
 * VL Nigeria (batch SECNGFIX_*, cf. scripts/fix/sec_ng_apply_corrections.py).
 *
 * POURQUOI CE SCRIPT
 * ------------------
 * La correction Nigeria a modifie `value` sur 27 660 lignes (decalage prouve),
 * rempli les mesures explicites sur 44 434 lignes et insere 23 731 lignes
 * manquantes. Tout ce qui DERIVE de `value` est donc devenu obsolete pour ce
 * pays uniquement :
 ...
 *
 * USAGE
 *   node scripts/recalc/recalc_nigeria_after_correction.js
 *       -> DRY-RUN : etat avant + plan detaille, AUCUNE ecriture
 *
 *   node scripts/recalc/recalc_nigeria_after_correction.js --execute --confirm
 *       -> execute les etapes 1 a 4 (Nigeria uniquement)
 *
 *   node scripts/recalc/recalc_nigeria_after_correction.js --execute --confirm --with-classements
 *       -> + etape 5 (impact inter-pays assume)
 *
 *   --pays TUNISIE   -> reutilisable pour un autre pays (defaut : NIGERIA)
 *
 * NON-DESTRUCTIF : ce script n'ecrit rien lui-meme. Il orchestre des scripts
 * deja valides en production, tous restreints au pays cible.
 */
'use strict';
...
function parseArgs() {
  ...
  return {
    pays: paysIdx !== -1 && a[paysIdx + 1] ? a[paysIdx + 1] : 'NIGERIA',
...
/** Photo de l'etat derive du pays : ce que le site affiche reellement. */
async function snapshot(conn, pays) {
  const [[fonds]] = await conn.execute(
    `SELECT COUNT(*) AS n FROM fond_investissements
     WHERE active = 1 AND LOWER(pays) = LOWER(?)`, [pays]);
  ...
  const [[vl]] = await conn.execute(
    `SELECT COUNT(*) AS n, MAX(v.date) AS derniere_date,
     ...
     WHERE f.active = 1 AND LOWER(f.pays) = LOWER(?)`, [pays]);
  ...
  const [decales] = await conn.execute(
    `SELECT f.id, f.nom_fond,
     ...
     WHERE f.active = 1 AND LOWER(f.pays) = LOWER(?)
     GROUP BY f.id, f.nom_fond
  ...
  return { fonds: fonds.n, vl, perf, decalesN: decalesN.n, decales };
}
...
async function main() {
  ...
  console.log('==========================================================');
  console.log('  RECALCUL CIBLE APRES CORRECTION DES VL');
  console.log(`  Pays : ${P}`);
  ...
  const avant = await snapshot(conn, P);
  ...
  const plan = [
    ['vl_ajuste = value + cumul dividendes', scripts.vlAjuste, ['--pays', P]],
    ['value_EUR / value_USD au taux du jour', scripts.eurUsd, ['--pays', P]],
    ['performances locales (YTD, 1A, 3A...)', scripts.perf, ['--pays', P, '--force']],
    ['performances EUR + USD', scripts.perfDev, ['--devise', 'BOTH', '--pays', P, '--force']],
  ];
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    if (!runStep(i + 1, plan.length, titre, script, args)) {
      ...
      const partiel = await snapshot(conn, P);
      printSnapshot('APRES (PARTIEL)', partiel, P);
      await conn.end();
      process.exit(1);
    }
  }

  const apres = await snapshot(conn, P);
  printSnapshot('APRES', apres, P);
